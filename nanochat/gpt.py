...
class GPTConfig:
    ...
    gate_min: float = 0.0        # leaky gate floor: g = gate_min + (1-gate_min)*sigmoid(...) ∈ [gate_min, 1]; prevents g=0 absorbing state
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        # Gate projection: g = gate_min + (1-gate_min)*sigmoid(gate_proj(cat([u, s]))) ∈ [gate_min, 1]
        # cat([u,s]) lets the gate compare proposed vs current state in full vector space (richer than scalar norm(u-s)).
        # gate_min>0 prevents sigmoid saturation at g=0 (the absorbing dead-zone that caused v22/v23 collapse).
        # bias=+2 init: gates start open (≈0.88+gate_min*(1-0.88)) so model starts near full recurrence.
    ...
    def forward(self, idx, targets=None, kv_cache=None, loss_reduction='mean', num_recur=None,
        ...
        if not self.training:
            self._last_gate_mean = gate_cost.detach().item()  # store for eval logging
        ...
        if targets is not None:
            # training mode: return (loss, gate_mean) where gate_mean in [0,1] = avg gate openness
