...
class Engine:
    ...
    def generate(
        ...
        """EBT generation: full forward pass each step (no KV cache)."""
        ...
        while True:
            ...
            # Build batch of token sequences (one per sample)
            seqs = [state.current_tokens for state in row_states]
            ...
            # Forward the model (full sequence, no KV cache)
            logits = self.model.forward(ids)  # (B, T, vocab_size)
